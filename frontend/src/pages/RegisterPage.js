...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(`${API}/auth/register`, form);
      login(data); toast.success(`Account created! Welcome, ${data.name}!`); navigate('/');
      ...
      <div className="w-full max-w-sm">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-7">
          <div className="text-center mb-7">
            <div className="w-12 h-12 bg-emerald-600 rounded-xl flex items-center justify-center text-white text-xl font-bold mx-auto mb-4 shadow-sm">K</div>
            ...
            {fields.map(field => (
              <div key={field.key}>
                <label className="block text-xs font-semibold text-gray-700 mb-1.5">
                  {field.label}
                  {field.optional && <span className="ml-1.5 text-[10px] font-normal text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded-md">Optional</span>}
                </label>
                <input
