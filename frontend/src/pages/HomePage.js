...
const CATEGORIES = ['All', 'Atta & Rice', 'Dal & Pulses', 'Oil & Ghee', 'Spices', 'Snacks', 'Dairy', 'Beverages', 'Soap & Cleaning', 'Other'];
const CATEGORY_ICONS = {
  'All': '🛍️', 'Atta & Rice': '🌾', 'Dal & Pulses': '🫘', 'Oil & Ghee': '🫙',
  'Spices': '🌶️', 'Snacks': '🍿', 'Dairy': '🥛', 'Beverages': '☕',
  'Soap & Cleaning': '🧼', 'Other': '📦',
};
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex flex-row sm:flex-col gap-2 shrink-0">
              {[['📍', '1 KM Zone'], ['💵', 'Cash on Delivery'], ['⚡', '30–45 Min']].map(([icon, label]) => (
                <div key={label} className="flex items-center gap-1.5 text-xs font-medium text-gray-600 bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-md whitespace-nowrap">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        {/* Search */}
        <div className="my-5">
          <div className="flex items-center gap-2.5 bg-white border border-gray-200 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-50 rounded-lg px-3.5 h-11 transition-all duration-150">
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} className="text-gray-400 hover:text-gray-600 w-5 h-5 rounded-full flex items-center justify-center text-xs hover:bg-gray-100 transition-all">✕</button>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-3 mb-5" style={{ scrollbarWidth: 'none' }}>
          {CATEGORIES.map(cat => (
            <button
              key={cat}
              onClick={() => setCategory(cat)}
              className={`whitespace-nowrap flex items-center gap-1 px-3 py-1.5 rounded-md text-xs font-semibold shrink-0 border transition-all duration-150 ${
                category === cat
        ...
        {!search && (
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-semibold text-gray-900">{category === 'All' ? 'All Products' : category}</h2>
              {!loading && <p className="text-xs text-gray-400 mt-0.5">{filtered.length} items available</p>}
            </div>
          </div>
        )}
        {search && (
          <div className="mb-4">
            <p className="text-sm text-gray-600">
              Results for <span className="font-semibold text-blue-600">"{search}"</span>
              {!loading && <span className="text-gray-400"> · {filtered.length} found</span>}
            </p>
          </div>
        )}
        ...
        {/* Products Grid */}
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 pb-16">
            {[...Array(10)].map((_, i) => <div key={i} className="h-64 rounded-xl bg-gray-200 animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-24 px-4">
