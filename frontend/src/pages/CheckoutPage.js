...
export default function CheckoutPage() {
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { setLocation({ lat: pos.coords.latitude, lng: pos.coords.longitude }); setLocLoading(false); toast.success('Location detected!'); },
      () => { setLocError('Could not get your location. Please allow location access.'); setLocLoading(false); },
      { timeout: 10000 }
    );
  };
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-0.5">Confirm your delivery details below.</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-5 items-start">
          ...
          <div className="flex-1 space-y-3">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-semibold shrink-0">1</div>
                <div>
                  <h3 className="font-semibold text-gray-900 text-sm">Your Location</h3>
                  <p className="text-xs text-gray-400">Needed to verify 1 km delivery zone</p>
                </div>
              </div>
              {location ? (
                <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="font-semibold text-emerald-700 text-xs">Location captured</p>
                    <p className="text-[10px] text-emerald-600 mt-0.5">{location.lat.toFixed(5)}, {location.lng.toFixed(5)}</p>
                  </div>
                </div>
              ) : (
                <>
                  {locError && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3">
                      <p className="text-red-600 text-xs font-medium">{locError}</p>
                    </div>
                  )}
                  <button
                    onClick={getLocation}
                    disabled={locLoading}
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold py-2.5 rounded-md text-sm transition-all duration-150 shadow-sm"
                  >
                    {locLoading ? 'Getting location...' : '📍 Share My Location'}
                  </button>
                </>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-semibold shrink-0">2</div>
                <div>
              ...
              <textarea
                ...
                value={address}
                onChange={e => setAddress(e.target.value)}
                className="w-full px-3.5 py-2.5 border border-gray-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-50 rounded-lg text-sm resize-none bg-gray-50 focus:bg-white transition-all duration-150 outline-none"
              />
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 text-xs font-semibold shrink-0">3</div>
                <div>
                  <h3 className="font-semibold text-gray-900 text-sm">
                    Order Note <span className="ml-1.5 text-[10px] font-normal text-gray-400 bg-gray-100 px-2 py-0.5 rounded-md">Optional</span>
                  </h3>
                  <p className="text-xs text-gray-400">Any special instructions for delivery</p>
                </div>
              </div>
              <input
                ...
                value={note}
                onChange={e => setNote(e.target.value)}
                className="w-full px-3.5 py-2.5 border border-gray-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-50 rounded-lg text-sm bg-gray-50 focus:bg-white transition-all duration-150 outline-none"
              />
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <div className="flex items-center gap-3 bg-gray-50 border-2 border-blue-500 rounded-lg p-3.5">
                <div className="w-9 h-9 bg-white rounded-lg flex items-center justify-center text-lg border border-gray-200 shrink-0">💵</div>
                <div className="flex-1">
          ...
          <div className="w-full lg:w-72 shrink-0 bg-white rounded-xl border border-gray-200 p-5 sticky top-16">
            ...
            <div className="space-y-2 mb-4 max-h-44 overflow-y-auto">
              {cart.map(item => (
                <div key={item._id} className="flex justify-between items-center text-sm gap-2">
                  ...
                  <span className="font-semibold text-gray-900 text-xs shrink-0">₹{item.price * item.quantity}</span>
                </div>
              ))}
            </div>
            <div className="border-t border-gray-100 pt-3 space-y-1.5 text-sm">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold text-xs">₹{cartTotal}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-500 text-xs">Delivery</span>
                <span className="font-semibold text-xs text-emerald-600">FREE</span>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg text-center py-2 text-xs text-amber-700 font-medium mt-4">
              Estimated: 30–45 minutes
            </div>
            <button
              onClick={handleOrder}
              disabled={!canPlace}
              className="w-full mt-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-md text-sm transition-all duration-150 shadow-sm"
            >
              {placing ? 'Placing Order...' : `Place Order · ₹${cartTotal}`}
            </button>
            {!canPlace && !placing && (
